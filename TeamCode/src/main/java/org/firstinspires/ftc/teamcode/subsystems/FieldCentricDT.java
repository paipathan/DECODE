...
public class FieldCentricDT {
    ...
    public FieldCentricDT(HardwareMap hwMap, Gamepad gamepad) {
    ...
    public void loop() {
        ...
        if (alignToGoal) {
            ...
            // --- FIELD CENTRIC CONVERSION START ---
            ...
            double botHeading = follower.getPose().getHeading(); // current heading (radians)

            // Field-centric transformation
            double rotX = x * Math.cos(-botHeading) - y * Math.sin(-botHeading);
            double rotY = x * Math.sin(-botHeading) + y * Math.cos(-botHeading);

            // Optional correction for imperfect strafing
            rotX *= 1.1;

            // Pass the rotated inputs to PedroPathing follower
            follower.setTeleOpDrive(rotY, rotX, rx, true);
            // --- FIELD CENTRIC CONVERSION END ---
        }
    }
    ...
    private void driveWithHeadingLock() {
        ...
        // still field-centric while aligning
        ...
        follower.setTeleOpDrive(rotY, rotX, rotationPower, true);
    }
